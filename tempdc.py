...
AC bias removal is done in HARDWARE via the iq0 module configured at 0 Hz
(DC demodulation = pure low-pass filter). The scope reads the iq0 X output
instead of raw in1, so the 500 Hz AC is stripped before any data hits Python.
No scipy edge effects, no lost endpoints.
...
INPUT_MODE = 'MANUAL'
MANUAL_GAIN_FACTOR = -1   #28.93002007*-1
MANUAL_DC_OFFSET = 0         #-0.016903
...
# ============================================================
# AC BIAS REMOVAL  (hardware IQ low-pass filter at 0 Hz)
# ============================================================
# iq0 is configured as a DC demodulator (frequency = 0 Hz).
# Its X output is simply in1 passed through a low-pass filter
# with bandwidth = AC_REMOVAL_CUTOFF Hz.  Because filtering
# happens inside the FPGA there are no edge effects and no
# data points are ever lost or distorted.
#
# AC_REMOVAL_CUTOFF must be:
#   (a) well BELOW  your AC signal frequency  (500 Hz) -- to reject it
#   (b) well ABOVE  your DC ramp bandwidth    (< 1 Hz) -- to keep it
#
# 7 Hz is a good default for a 500 Hz AC signal on a slow ramp.
AC_REMOVAL_ENABLED = True
AC_REMOVAL_CUTOFF  = 7      # Hz -- MUST be well below your AC frequency (500 Hz)
# ============================================================
...
class RedPitayaDCLogger:
    ...
    When AC_REMOVAL_ENABLED is True the iq0 module is configured as a
    hardware low-pass filter (frequency=0 Hz, bandwidth=AC_REMOVAL_CUTOFF).
    The scope is pointed at iq0 so only the filtered DC reaches the buffer.
    When AC_REMOVAL_ENABLED is False the scope reads raw in1 exactly as before.
    All other functionality (gain, offset, calibration, CSV, plot) is unchanged.
    """
    ...
    def __init__(self, output_dir='test_data', input_mode='AUTO',
        ...
        self.iq0    = self.rp_modules.iq0   # used as hardware DC low-pass filter
        ...
        # Scope decimation (same as before)
    ...
    # ------------------------------------------------------------------ #
    # Hardware IQ low-pass filter setup / teardown
    # ------------------------------------------------------------------ #

    def _setup_iq_filter(self, cutoff_hz):
        """Configure iq0 as a DC (0 Hz) low-pass filter and point scope at it."""
        print(f"\nConfiguring iq0 hardware low-pass filter @ {cutoff_hz} Hz...")
        self.iq0.setup(
            frequency=0,           # DC demodulation -- X output = low-passed in1
            bandwidth=cutoff_hz,   # low-pass cutoff
            gain=0.0,
            phase=0,
            acbandwidth=0,
            amplitude=0,           # iq0 does NOT generate any output signal
            input='in1',
            output_direct='off',   # iq0 does NOT drive out1/out2
            output_signal='quadrature',
            quadrature_factor=1.0,
        )
        # X channel of iq0 is the in-phase (DC) component -- scope reads it
        self.scope.input1 = 'iq0'
        self.scope.input2 = 'iq0'
        actual_bw = self.iq0.bandwidth
        tc_ms = 1e3 / (2 * np.pi * cutoff_hz)
        print(f"  iq0 bandwidth:  {cutoff_hz} Hz requested  |  {actual_bw} Hz actual")
        print(f"  Time constant:  {tc_ms:.1f} ms")
        print(f"  Scope input1/2: iq0  (hardware-filtered DC)")

    def _teardown_iq_filter(self):
        """Restore scope to raw in1 after measurement."""
    ...
    def calibrate_input_gain(self, cal_freq=100, cal_amp=1.0, cal_time=2.0, force=False):
        ...
        # Calibration always reads raw in1 so gain numbers are correct
    ...
    def run(self, params):
        ...
        # ── Hardware IQ filter setup ──────────────────────────────────────────
        ac_enabled = params.get('ac_removal_enabled', False)
        ac_cutoff  = params.get('ac_removal_cutoff', 50)
        ac_label   = ''

        if ac_enabled:
            self._setup_iq_filter(ac_cutoff)
            ac_label = f' [HW LP {ac_cutoff} Hz via iq0]'
            # Allow the IQ filter to settle: ~5 time constants = 5/(2*pi*cutoff)
            settle_time = max(0.3, 5.0 / (2 * np.pi * ac_cutoff))
            print(f"  Waiting {settle_time:.2f}s for IQ filter to settle...")
            time.sleep(settle_time)
        else:
            # Raw in1 -- identical behaviour to original code
            self.scope.input1 = 'in1'
            self.scope.input2 = 'in1'
        # ─────────────────────────────────────────────────────────────────────
        ...
        # Restore scope to raw in1 after acquisition
        if ac_enabled:
            self._teardown_iq_filter()
        ...
        if ac_enabled:
            print(f"\nAC bias removal: HARDWARE iq0 low-pass @ {ac_cutoff} Hz")
            print(f"  No post-processing filter needed -- FPGA handled it.")
        # ─────────────────────────────────────────────────────────────────────
        ...
        print(f"DC offset:     {self.input_dc_offset:.6f}V")
        print(f"AC removal:    {'HW iq0 LP -- cutoff ' + str(ac_cutoff) + ' Hz' if ac_enabled else 'OFF'}")
        ...
        plt.fill_between(t_p,
                         ...
                         label=f'+/-1σ ({np.std(corrected):.6f} V)')
        plt.xlabel("Time (s)")
        ...
        if params['save_file']:
            ...
            with open(csv_path, 'w', newline='', encoding='ascii') as f:
                ...
                f.write(f"# AC removal: {'HW iq0 LP cutoff=' + str(ac_cutoff) + ' Hz' if ac_enabled else 'disabled'}\n")
...
if __name__ == "__main__":
    ...
    run_params = {
        ...
        # AC bias removal -- now handled in hardware via iq0
    ...
    print(f"AC bias removal:  {'ON -- HW iq0 LP cutoff ' + str(AC_REMOVAL_CUTOFF) + ' Hz (FPGA, no edge effects)' if AC_REMOVAL_ENABLED else 'OFF'}")
