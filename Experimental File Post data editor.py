...
// Up/down boolean masks (parallel to rawDC_V_full)
let rawUpMask = null;
let rawDownMask = null;
...
function classifyTraceObj(tr) {
  ...
  // Sweep scatter plots (DC-vs-voltage x-axis).
  // Distinguish R vs Theta by yaxis column parity.
  if (nm === 'forward sweep' || nm === 'reverse sweep') {
    ...
    return { sig, xtype: 'dc_voltage', dir };
  }
...
function extractRaw() {
  ...
  // Derive up/down masks from DC voltage using derivative sign
  if (rawDC_V_full) {
    rawUpMask   = new Uint8Array(N);
    rawDownMask = new Uint8Array(N);
    for (let i = 0; i < N - 1; i++) {
      if (rawDC_V_full[i+1] > rawDC_V_full[i]) rawUpMask[i]   = 1;
      else                                       rawDownMask[i] = 1;
    }
    rawUpMask[N-1]   = rawUpMask[N-2];
    rawDownMask[N-1] = rawDownMask[N-2];
  }
...
// ------------------------------------------------------------
// rerender — apply state to ALL traces in one batched Plotly.restyle
// Fast: only two restyle calls total (one per update)
// ------------------------------------------------------------
function rerender() {
  ...
  const end = N - R;   // exclusive upper bound (for time-domain)

  // We'll batch: indices[], xArrays[], yArrays[]
  const batchIdx = [];
  const batchX   = [];
  const batchY   = [];

  rawData.forEach(raw => {
    if (!raw.sig) return;   // sigma bands, unlabelled — leave alone
    ...
    // ---- DC-VS-VOLTAGE SCATTER TRACES --------------------------------
    // These are originally plotted as (DC_voltage[i], signal[i]).
    // When shift is applied to a signal, we need to re-pair:
    //   x = DC_voltage[i]          (not shifted — DC position stays)
    //   y = signal[i - sh] * gain  (signal reads from shifted index)
    // Crop and removed-point logic also applies.
    } else if (xtype === 'dc_voltage') {
      if (!rawDC_V_full || !rawTimeSeries[sig]) {
        return;
      }

      const sigY = rawTimeSeries[sig].y;
      const mask = (dir === 'up') ? rawUpMask : rawDownMask;
      ...
      for (let i = L; i < Math.min(end, N); i++) {
        if (!mask[i]) continue;             // wrong sweep direction
        if (state.removed.has(i)) continue; // removed point — skip entirely (scatter, no null gap needed)

        const src = i - sh;
        if (src < 0 || src >= sigY.length) continue;

        const rawSigY = sigY[src];
        if (rawSigY === undefined || isNaN(rawSigY)) continue;

        const dcSrc = i - dcShift;
        const dcV = (dcSrc >= 0 && dcSrc < rawDC_V_full.length) ? rawDC_V_full[dcSrc] : undefined;
        if (dcV === undefined || isNaN(dcV)) continue;
  ...
  if (batchIdx.length) {
    try {
      Plotly.restyle(div, { x: batchX, y: batchY }, batchIdx);
    } catch(e) {
...
<html><head><meta charset="utf-8"><title>ACCV Edited Export</title>
