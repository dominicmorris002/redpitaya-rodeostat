"""
Red Pitaya Lock-In Amplifier - MASTER (with Hardware Sync)

This is the MASTER Red Pitaya that:
1. Generates the lock-in reference signal
2. Sends trigger signals to slave Red Pitayas via SATA daisy-chain
3. Synchronizes scope acquisitions across all boards

SATA Connection: Connect "Clock and Trigger" SATA cable from this board to slave(s)
...
# ============================================================
HARDWARE_SYNC = True  # Use SATA cable synchronization
TRIGGER_OUTPUT = True  # Send triggers to slave boards
# ============================================================


class RedPitayaLockInMaster:
    """Lock-in amplifier with hardware synchronization support"""

    allowed_decimations = [1, 8, 64, 1024, 8192, 65536]

    def __init__(self, output_dir='test_data', input_mode='AUTO', manual_gain=1.0, 
                 manual_offset=0.0, hardware_sync=False):
        
        self.rp = Pyrpl(config='lockin_master_config', hostname='rp-f073ce.local')
        ...
        if self.hardware_sync:
            print("\n" + "=" * 60)
            print("CONFIGURING HARDWARE SYNCHRONIZATION (MASTER)")
            print("=" * 60)
            # Set this board as the clock master
            # This sends the clock signal to slave boards via SATA cable
            try:
                # Access the Red Pitaya's direct register interface
                self.rp_modules.hk.led = 0b11111111  # Turn on all LEDs to indicate master
                print("✓ Board configured as CLOCK MASTER")
                print("  Clock signal will be sent to slave boards via SATA")
                print("  Trigger signals will synchronize scope acquisitions")
            except Exception as e:
                print(f"⚠ Warning: Could not configure as master: {e}")
        ...
        if self.hardware_sync:
            # Use external trigger from the lock-in's internal signal
            # This will cause slaves to trigger at the same time
            self.scope.trigger_source = 'immediately'  # Master generates its own triggers
            print("Scope trigger: MASTER MODE (generates triggers)")
        else:
    ...
    def setup_lockin(self, params):
        ...
        if self.hardware_sync:
            print(f"Hardware sync: ENABLED (master mode)")
    ...
    def run(self, params):
        ...
        print("\n" + "=" * 60)
        print("LOCK-IN RESULTS (MASTER)")
        print("=" * 60)
        print(f"Mode: {self.input_mode}")
        print(f"Gain correction: {self.input_gain_factor:.4f}x")
        print(f"DC offset correction: {self.input_dc_offset:.6f}V")
        print(f"Hardware sync: {'ENABLED (master)' if self.hardware_sync else 'DISABLED'}")
        ...
        margin_R = 5 * (np.max(R * 1e6) - np.min(R * 1e6)) if np.max(R * 1e6) != np.min(R * 1e6) else 0.001
        ...
        margin_Theta = 5 * (np.max(Theta) - np.min(Theta)) if np.max(Theta) != np.min(Theta) else 0.001
        ...
        if params['save_file']:
            ...
            with open(csv_path, 'w', newline='') as f:
                f.write(f"# Red Pitaya Lock-In Amplifier Data Logger (MASTER)\n")
                f.write(f"# Mode: {self.input_mode}\n")
                f.write(f"# Hardware sync: {'ENABLED (master)' if self.hardware_sync else 'DISABLED'}\n")
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("RED PITAYA LOCK-IN DATA LOGGER (MASTER)")
    print("=" * 60)
