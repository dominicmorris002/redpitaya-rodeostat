...
WITH SOFTWARE-SYNCHRONIZED TIMESTAMP ALIGNMENT
...
This version synchronizes both instruments to start at the exact same time.
"""
...
class RedPitayaLockIn:
    ...
    def run_synchronized(self, params, sync_start_time):
        """Run acquisition with synchronized start time"""
        timeout = params['timeout']
        
        # Store the synchronized start time
        self.acquisition_start_time = sync_start_time
        
        # Wait until the synchronized start time
        while time.time() < sync_start_time:
            time.sleep(0.0001)  # 100 microsecond polling
        
        print(f"✓ Lock-in started at: {datetime.fromtimestamp(sync_start_time).strftime('%Y-%m-%d %H:%M:%S.%f')}")
...
class RedPitayaDCMonitor:
    ...
    def run_synchronized(self, params, sync_start_time):
        ...
        print(f"✓ DC Monitor started at: {datetime.fromtimestamp(sync_start_time).strftime('%Y-%m-%d %H:%M:%S.%f')}")
...
if __name__ == '__main__':
    print("=" * 60)
    print("SYNCHRONIZED RED PITAYA MONITOR")
    print("=" * 60)
    ...
    # Run calibration if needed (before sync)
    if LOCKIN_AUTO_CALIBRATE:
        rp_lockin.calibrate_input_gain(
            cal_freq=LOCKIN_REF_FREQUENCY,
            cal_amp=LOCKIN_REF_AMPLITUDE,
            cal_time=LOCKIN_CALIBRATION_TIME
        )

    # Setup lock-in (before sync)
    rp_lockin.setup_lockin(lockin_params)
    ...
    print("Waiting for DC monitor to settle...")
    time.sleep(0.1)
    ...
    # ============================================================
    print("\n" + "=" * 60)
    print("PREPARING SYNCHRONIZED START")
    print("=" * 60)
    
    # Schedule start time 1 second in the future
    sync_start_time = time.time() + 1.0
    
    print(f"Both instruments will start at: {datetime.fromtimestamp(sync_start_time).strftime('%Y-%m-%d %H:%M:%S.%f')}")
    print("Synchronizing...")
    
    # Import threading to run both simultaneously
    import threading
    
    # Create threads for both instruments
    lock_thread = threading.Thread(target=rp_lockin.run_synchronized, args=(lockin_params, sync_start_time))
    dc_thread = threading.Thread(target=rp_dc.run_synchronized, args=(dc_params, sync_start_time))
    ...
    sync_offset = abs(rp_lockin.acquisition_start_time - rp_dc.acquisition_start_time)
    print(f"Synchronization offset: {sync_offset * 1000:.3f} ms")
    if sync_offset < 0.001:  # Less than 1ms
        print("✓ Excellent synchronization (<1ms offset)")
    elif sync_offset < 0.01:  # Less than 10ms
        print("✓ Good synchronization (<10ms offset)")
    else:
        print(f"⚠ Synchronization offset is {sync_offset*1000:.1f}ms")
    ...
    # Build combined CSV with padding
    ...
    print("\n" + "=" * 60)
    print("SYNCHRONIZED MEASUREMENT COMPLETE")
    print("=" * 60)
