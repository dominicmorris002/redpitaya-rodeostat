"""
Combined Red Pitaya Lock-In Amplifier and DC Voltage Monitor with Daisy Chain Synchronization

SETUP:
- Red Pitaya 1 (rp-f073ce.local): Lock-in amplifier (PRIMARY) with OUT1 connected to IN1
- Red Pitaya 2 (rp-f0909c.local): DC voltage monitor (SECONDARY) on IN1
- Daisy chain cable connecting the two units for clock and trigger synchronization

IMPORTANT: Connect daisy chain cable between units before running!
"""
...
# ============================================================
ENABLE_DAISY_CHAIN = True  # Set to False to disable synchronization
PRIMARY_IP = 'rp-f073ce.local'
SECONDARY_IP = 'rp-f0909c.local'
...
def setup_daisy_chain(primary_ip, secondary_ip):
    """
    Configure daisy chain synchronization between two Red Pitaya units.
    This must be done BEFORE initializing PyRPL to ensure proper synchronization.
    
    Returns:
        tuple: (rp_prim, rp_sec) SCPI connection objects (or None if disabled)
    """
    try:
        import redpitaya_scpi as scpi
    except ImportError:
        print("=" * 60)
        print("ERROR: redpitaya_scpi module not found!")
        print("Install with: pip install redpitaya-scpi")
        print("=" * 60)
        return None, None
    
    print("\n" + "=" * 60)
    print("CONFIGURING DAISY CHAIN SYNCHRONIZATION")
    print("=" * 60)
    
    # Connect to both units
    print(f"Connecting to PRIMARY unit: {primary_ip}")
    rp_prim = scpi.scpi(primary_ip)
    
    print(f"Connecting to SECONDARY unit: {secondary_ip}")
    rp_sec = scpi.scpi(secondary_ip)
    
    # Reset both units
    print("\nResetting both units...")
    rp_prim.tx_txt('GEN:RST')
    rp_prim.tx_txt('ACQ:RST')
    rp_sec.tx_txt('GEN:RST')
    rp_sec.tx_txt('ACQ:RST')
    
    # Configure PRIMARY unit for daisy chain
    print("\nConfiguring PRIMARY unit daisy chain settings...")
    rp_prim.tx_txt('DAISY:SYNC:TRig ON')    # Enable trigger sync
    rp_prim.tx_txt('DAISY:SYNC:CLK ON')     # Enable clock sync
    rp_prim.tx_txt('DAISY:TRIG_O:SOUR ADC') # Trigger source is ADC
    rp_prim.tx_txt('DIG:PIN LED5,1')        # LED indicator
    
    time.sleep(0.2)
    
    # Verify PRIMARY settings
    trig_status = rp_prim.txrx_txt('DAISY:SYNC:TRig?')
    clk_status = rp_prim.txrx_txt('DAISY:SYNC:CLK?')
    sour_status = rp_prim.txrx_txt('DAISY:TRIG_O:SOUR?')
    
    print(f"  Trigger Sync: {trig_status}")
    print(f"  Clock Sync: {clk_status}")
    print(f"  Trigger Source: {sour_status}")
    
    # Configure SECONDARY unit for daisy chain
    print("\nConfiguring SECONDARY unit daisy chain settings...")
    rp_sec.tx_txt('DAISY:SYNC:TRig ON')     # Enable trigger sync
    rp_sec.tx_txt('DAISY:SYNC:CLK ON')      # Enable clock sync
    rp_sec.tx_txt('DAISY:TRIG_O:SOUR ADC')  # External trigger will trigger ADC
    rp_sec.tx_txt('DIG:PIN LED5,1')         # LED indicator
    
    time.sleep(0.2)
    
    print("\n✓ Daisy chain synchronization configured successfully!")
    print("  - Both units are now synchronized")
    print("  - Clock signals are shared")
    print("  - Triggers are synchronized")
    print("  - LED5 lit on both units indicates sync is active")
    print("=" * 60)
    
    # Return connection objects in case they're needed later
    return rp_prim, rp_sec
...
class RedPitayaLockIn:
    ...
    def __init__(self, output_dir='test_data', input_mode='AUTO', manual_gain=1.0):
        print("\nInitializing PyRPL for lock-in amplifier...")
...
class RedPitayaDCMonitor:
    ...
    def __init__(self, output_dir='test_data'):
        print("\nInitializing PyRPL for DC monitor...")
...
if __name__ == '__main__':
    print("=" * 60)
    print("COMBINED RED PITAYA MONITOR WITH DAISY CHAIN")
    print("=" * 60)
    
    # Step 1: Configure daisy chain BEFORE initializing PyRPL
    rp_prim_scpi = None
    rp_sec_scpi = None
    
    if ENABLE_DAISY_CHAIN:
        rp_prim_scpi, rp_sec_scpi = setup_daisy_chain(PRIMARY_IP, SECONDARY_IP)
        
        if rp_prim_scpi is None:
            print("\n⚠ WARNING: Daisy chain setup failed!")
            print("Continuing without synchronization...")
            ENABLE_DAISY_CHAIN = False
        else:
            print("\n✓ Daisy chain configured - units are synchronized")
            print("  You can now proceed with PyRPL initialization")
    else:
        print("\n⚙ Daisy chain synchronization is DISABLED")
        print("  Units will run independently")
    
    # Step 2: Initialize PyRPL for both units
    print("\n" + "=" * 60)
    ...
    if ENABLE_DAISY_CHAIN:
        print("Note: Units are synchronized via daisy chain")
        print("      Acquisitions should be time-aligned")
    
    print("\nRunning Lock-In Amplifier...")
    ...
    # Step 4: Clean up SCPI connections if they were opened
    if ENABLE_DAISY_CHAIN and rp_prim_scpi is not None:
        print("\nClosing SCPI connections...")
        try:
            rp_prim_scpi.close()
            rp_sec_scpi.close()
            print("✓ SCPI connections closed")
        except:
            pass
    ...
    t_lock_abs = rp_lockin.sample_timestamps
    t_dc_abs = rp_dc.sample_timestamps

    t_lock_rel = rp_lockin.t
    t_dc_rel = rp_dc.sample_timestamps - rp_dc.acquisition_start_time
    ...
    # Build combined CSV with padding if needed
    ...
    if SAVE_DATA:
        ...
        if ENABLE_DAISY_CHAIN:
            print("  Note: Time alignment should be improved due to daisy chain sync")
    ...
    if ENABLE_DAISY_CHAIN:
        print("✓ Daisy chain synchronization was active")
        print("  Clock and trigger signals were shared between units")
    print("=" * 60)
