"""
Synchronous dual Red Pitaya + Rodeostat data acquisition - ACCV Style
Plots styled for AC Cyclic Voltammetry visualization

The Rodeostat runs in sync to generate the voltage ramp.
Its CSV is saved but its data is NOT used in the RP analysis --
the Red Pitayas are the sole source of measurement data.

Press Enter ONCE to launch all three subprocesses.
Press Enter a SECOND time when both RPs are connected and the
Rodeostat is ready -- fires all three at the exact same instant.

Plots are saved as PNG (no popups) and as interactive HTML.
...
print("  Dual Red Pitaya  +  Rodeostat (voltage source only)")
print("=" * 60)
...
print("  Watch for both RPs to connect AND the Rodeostat to print 'Waiting...' below...")
print("")
...
# Read only Red Pitaya CSVs -- Rodeostat CSV is saved but ignored here
try:
    ...
    print("Error: Could not find Red Pitaya output files!")
    sys.exit(1)

try:
    rodeostat_csv = max(glob.glob(os.path.join('rodeostat_data', '*_data_*.csv')), key=os.path.getctime)
    print(f"\nRodeostat CSV saved (not used in RP analysis): {rodeostat_csv}")
except ValueError:
    print("\nNote: Rodeostat CSV not found (may still be writing)")

print("\n" + "=" * 60)
print("MERGING RED PITAYA DATA (INDEX-BASED)")
print("=" * 60)
...
# 7-panel RP-only layout (saved silently)
#   Row 0: R vs Time | Theta vs Time | DC vs Time
...
ax_R_t_dc.plot(li_time, dc_V, color='#E65100', linewidth=0.8, alpha=0.7)
...
ax_Th_t_dc.plot(li_time, dc_V, color='#E65100', linewidth=0.8, alpha=0.7)
...
fig.suptitle(
    f'AC Cyclic Voltammetry -- Synchronized Dual Red Pitaya Measurements\n'
...
# Interactive HTML (plotly) -- matches 7-panel PNG layout
try:
    ...
    fig_html = make_subplots(
        ...
        subplot_titles=[
            ...
            '',
            f'R vs DC Potential -- Scatter{ds_note}',
            f'Theta vs DC Potential -- Scatter{ds_note}',
            '',
        ],
        specs=[
    ...
    # Hide blank panels (row 2 col 3, row 3 col 3)
...
if Run_Cycle_Averaging:
    ...
    try:
        ...
        if os.path.exists(cycle_avg_script):
            ...
            # Open cycle-averaged HTML in browser
